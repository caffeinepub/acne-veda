...
import type { PatientDetails } from "./PatientDetailForm";
...
const DISCLAIMER = `This application is intended for informational and assistive purposes only. The e-prescriptions generated by this app are not a substitute for professional medical advice, diagnosis, or treatment.

Always consult a qualified healthcare provider before making any medical decisions. The developers of this application do not assume any responsibility for the accuracy, completeness, or outcomes of the generated prescriptions.

Use of this app does not establish a doctor–patient relationship. Any reliance on the information provided is strictly at your risk.`;
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .footer .disclaimer { font-style: italic; margin-bottom: 6px; white-space: pre-line; }
    .footer .gen { font-weight: 500; color: #374151; margin-top: 8px; }
    @media print {
...
<div class="page">
  <div class="header">
    ...
    <p>Ayurvedic Skin Care Platform</p>
    <p class="reg">Clear Skin Naturally</p>
  </div>
  <div class="body">
    <div class="meta">
      ${patientDetails?.name ? `<div><span class="label">Patient:</span> ${patientDetails.name}</div>` : patientName ? `<div><span class="label">Patient:</span> ${patientName}</div>` : ""}
      ${patientDetails?.sex ? `<div><span class="label">Sex:</span> ${patientDetails.sex}</div>` : ""}
      ${patientDetails?.age ? `<div><span class="label">Age:</span> ${patientDetails.age} yrs</div>` : ""}
      ${patientDetails?.dob ? `<div><span class="label">DOB:</span> ${patientDetails.dob}</div>` : ""}
      ${patientDetails?.occupation ? `<div><span class="label">Occupation:</span> ${patientDetails.occupation}</div>` : ""}
      ${patientDetails?.contact ? `<div><span class="label">Contact:</span> ${patientDetails.contact}</div>` : ""}
      <div><span class="label">Date:</span> ${date}</div>
      <div><span class="label">Diagnosis:</span> ${conditionLabel}</div>
      ${patientDetails?.address ? `<div style="flex-basis:100%;"><span class="label">Address:</span> ${patientDetails.address}</div>` : ""}
    </div>
    <div class="rx">&#8478;</div>
    ...
    <div class="footer">
      <p class="disclaimer">This application is intended for informational and assistive purposes only. The e-prescriptions generated by this app are not a substitute for professional medical advice, diagnosis, or treatment.\n\nAlways consult a qualified healthcare provider before making any medical decisions. The developers of this application do not assume any responsibility for the accuracy, completeness, or outcomes of the generated prescriptions.\n\nUse of this app does not establish a doctor–patient relationship. Any reliance on the information provided is strictly at your risk.</p>
          ...
          <div className="rounded-xl border border-green-200 overflow-hidden shadow-sm">
            ...
            <div className="bg-green-700 text-white px-6 py-4">
              ...
              <p className="text-sm mt-1 opacity-90">
                Ayurvedic Skin Care Platform
              </p>
              <p className="text-xs mt-0.5 opacity-70">Clear Skin Naturally</p>
            </div>
            ...
            <div className="bg-white px-6 py-4 space-y-4">
              ...
              <div className="flex flex-wrap justify-between gap-2 text-sm">
                ...
                {patientDetails?.sex && (
                  <div>
                    <span className="font-semibold text-green-700">Sex: </span>
                    <span className="text-gray-700">{patientDetails.sex}</span>
                  </div>
                )}
                {patientDetails?.age && (
                  <div>
                ...
                {patientDetails?.dob && (
                  <div>
                    <span className="font-semibold text-green-700">DOB: </span>
                    <span className="text-gray-700">{patientDetails.dob}</span>
                  </div>
                )}
              ...
              {/* Disclaimer */}
              <div className="text-xs text-gray-500 border-t border-dashed border-gray-200 pt-3 mt-2 space-y-2 bg-amber-50 rounded-lg p-3">
                <p className="font-semibold text-amber-800 text-xs uppercase tracking-wide">
                  Disclaimer
                </p>
                {DISCLAIMER.split("\n\n").map((para) => (
                  <p key={para.slice(0, 20)} className="italic leading-relaxed">
                    {para}
                  </p>
                ))}
                <p className="font-medium text-gray-500 pt-1">
